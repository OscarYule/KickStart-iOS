...
class HomeworkViewController: UIViewController {
    ...
    var model: HomeworkTable?
    var timeStr = ""

    override func viewDidLoad() {
        super.viewDidLoad()
        if (model != nil) {
            titLabel.text = "Edit Homework"
    ...
    @IBAction func saveAction(_ sender: Any) {
        ...
        if (model != nil) {
            ...
            HomeworkDataManage.updateNote(note: model!)
        }else {
            var noteTable = HomeworkTable()
            noteTable.subject = subjectField.text
            noteTable.time = timeStr
            noteTable.details = detailsField.text
            HomeworkDataManage.addNote(note:noteTable)
        }
